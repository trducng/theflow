...
## Define a Compose to get text from Wikipedia

A Compose is simply an operation that takes in input and returns an output.
Let's write the first Compose to retrieve the text from Wikipedia. It has an
...
class RetrieveArticle(Compose): # <1>
    ...
    def run(self, title: str) -> str:  # <3>
        ...
        return text    # <3>
```
1. Subclass `theflow.Compose` to create a flow.
2. Declare the init parameters with an explicit type annotation. `theflow` will
   automatically treat any parameters with type other than `theflow.Compose` as
   init parameters.
3. Declare the flow logic in `run`, with input and output type annotations. The
   ...
   a Compose is compatible with another Compose.
...
The relevant parameters will be added to the `RetrieveArticle` Compose above.
...
class RetrieveArticle(Compose):
    ...
    @Param.decorate(depends_on=["remove_tag_pattern"])    # <2>
...
Then, we can rerun this Compose to get the cleaner full text:
...
correct title, let's create a search Compose that can return relevant articles
given a search query.

```python
class WikipediaSearch(Compose):
...
`RetrieveArticle` and `WikipediaSearch` Compose.
...
class LearnFromWikipedia(Compose):
    search: Compose   # <1>
    retrieve: Node[Compose] = Node(                         # <2>
        default=RetrieveArticle,
        default_kwargs={"full_text":False}
    )                                                       # <2>
...
1. Declare a search node by annotating it with `Compose` type.
...
A node is defined by annotating it with `Compose` type or any of its sub-types.
...
it allows us to supply the default Compose and its parameters. Note, for node,
we don't want to set default by declaring:

```python
class LearnFromWikipedia(Compose):
    ...
    retrieve: Node[Compose] = RetrieveArticle(full_text=False)
    ...
```
...
- `.search`: This corresponds to the logged info of the search node, which
  defined by the `search: Compose` node in the flow.
