...
You can implicitly declare node by annotating the attributes with `Compose` type
...
attribute is accessed, not when the Compose object is initiated.

**With `default`**

When assinging a Node `default`, one can supply:

- A `Compose` object or any of its subclass. This object will be called `()` to
  initiate real instance once the node is accessed.
- If ones want the compose object (let's say named `ComponentA`) to be initiated
...
where `obj` will be supplied with parent Compose object, and `val` will be the
...
values in a Compose object. One supply an `auto_callback` function in the
...
nodes and params in the Compose changes. This value is set to True by default
...
- Pre-defined behavior changes: the default `Node` and `Param` can access
  certain pre-defined callbacks from the parent Compose.
...
- `_prepare_child(self, child: "Compose", name: str)` is called when a child
  node is being accessed. The `child` corresponds to the node object, and `name`
  corresponds to the node name in the parent compose.

#### Subclass way

After subclassing Node and Param, you can directly use them in your Compose
